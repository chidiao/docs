# 编程导航

## 素材

- [IconPark - 字节图标库](https://iconpark.oceanengine.com/)

- [Picsum - 占位图](https://picsum.photos/)

- [unDraw - 矢量图](https://undraw.co/illustrations)

- [Error404 - 模板页面](https://error404.fun/)

- [Dribbble - 设计灵感](https://dribbble.com/)

## CSS

- [Animista - CSS 动画](https://animista.net/)

- [Animate - CSS 动画](https://github.com/animate-css/animate.css)

- [ShapeDividers - 自定义形状](https://www.shapedivider.app/)

- [Normalize - CSS resets](https://github.com/necolas/normalize.css)

- [Hover - CSS](https://github.com/IanLunn/Hover/)

## 插件

- [ChinaDivision - 中国省市区数据](https://github.com/modood/Administrative-divisions-of-China)

- [uaDevice - UA 解析 设备识别](https://github.com/fex-team/ua-device)

- [Store.js - LocalStorage 封装](https://github.com/marcuswestin/store.js)

- [Compressor.js - 图片压缩](https://github.com/fengyuanchen/compressorjs)

- [AOS - 滚动动画](https://github.com/michalsnik/aos)

- [ScrollReveal - 滚动动画](https://github.com/jlmakes/scrollreveal)

## 工具

- [在线工具](https://tool.lu/)

- [Squoosh - 图片压缩](https://squoosh.app/)

- [Carbon - 代码截图](https://carbon.now.sh/)

## 玩具

- [Alpine.js - 轻量级框架](https://alpinejs.dev/)

## 其他

- [PxLoader](https://github.com/thinkpixellab/PxLoader)

- [FileSaver](https://github.com/eligrey/FileSaver.js)

- [SignaturePad](https://github.com/szimek/signature_pad)

- [BetterScroll](https://github.com/ustbhuangyi/better-scroll)

- [lax.js](https://github.com/alexfoxy/lax.js)

- [rellax.js](https://github.com/dixonandmoe/rellax)

- [parallax.js](https://github.com/wagerfield/parallax)

## 项目

[vanillawebprojects](https://github.com/bradtraversy/vanillawebprojects)
